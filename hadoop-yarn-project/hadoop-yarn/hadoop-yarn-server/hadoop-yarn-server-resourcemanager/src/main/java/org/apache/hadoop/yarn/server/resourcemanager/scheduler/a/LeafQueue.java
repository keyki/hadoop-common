...
public class LeafQueue implements CSQueue {
  ...
  public synchronized void trackApplications(ApplicationId applicationId, String user) {
    ((ParentQueue) getParent()).trackApplicationMovement(applicationId, user);
  }

  public synchronized void removeApplicationAttempt(FiCaSchedulerApp application) {
    removeApplicationAttempt(application, users.get(application.getUser()));
    getParent().finishApplication(application.getApplicationId(), application.getUser());
  }
  ...
  private synchronized boolean assignToQueue(Resource clusterResource,
    ...
    float potentialNewCapacity =
      Resources.divide(resourceCalculator, clusterResource, Resources.add(usedResources, required), clusterResource);
  ...
  private boolean unreserve(FiCaSchedulerApp application, Priority priority,
    ...
    if (application.unreserve(node, priority)) {
      ...
      getMetrics().unreserveResource(application.getUser(), rmContainer.getContainer().getResource());
      return true;
    }
    return false;
  }
