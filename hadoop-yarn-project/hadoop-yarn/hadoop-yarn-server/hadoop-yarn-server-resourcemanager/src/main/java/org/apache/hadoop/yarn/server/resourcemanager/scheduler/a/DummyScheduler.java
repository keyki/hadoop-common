...
public class DummyScheduler extends AbstractYarnScheduler
  implements PreemptableResourceScheduler, CapacitySchedulerContext,
  Configurable {

  private static final Log LOG = LogFactory.getLog(DummyScheduler.class);
  public static final String LOG_PREFIX = "############################################################################# ";
  public static final String LOG_PREFIX2 = "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ ";
  ...
  reinitialize(Configuration conf, RMContext rmContext) throws IOException {
    ...
    if (!initialized) {
      ...
      initialized = true;
      LOG.info(LOG_PREFIX + "Initialized DummyScheduler with " +
      ...
      validateConf(this.conf);
      try {
        LOG.info(LOG_PREFIX + "Re-initializing queues...");
  ...
  private void initializeQueues(CapacitySchedulerConfiguration conf)
    ...
    root =
      parseQueue(this, conf, null, CapacitySchedulerConfiguration.ROOT,
        queues, queues, noop);
    LOG.info(LOG_PREFIX + "Initialized root queue " + root);
  }
  ...
  static CSQueue parseQueue(
    ...
    LOG.info(LOG_PREFIX + "Initialized queue: " + queue);
    return queue;
  }
  ...
  private synchronized void addApplication(ApplicationId applicationId,
    ...
    try {
      ...
      LOG.info(LOG_PREFIX + "Failed to submit application " + applicationId + " to queue "
    ...
    LOG.info(LOG_PREFIX + "Accepted application " + applicationId + " from user: " + user
      + ", in queue: " + queueName);
  ...
  private synchronized void addApplicationAttempt(
    ...
    LOG.info(LOG_PREFIX + "Added Application Attempt " + applicationAttemptId
  ...
  private synchronized void doneApplication(ApplicationId applicationId,
    ...
    CSQueue queue = (CSQueue) application.getQueue();
    if (!(queue instanceof LeafQueue)) {
      LOG.error(LOG_PREFIX + "Cannot finish application " + "from non-leaf queue: "
        + queue.getQueueName());
    } else {
    ...
    DummyHistory.finished = true;
  }

  private synchronized void doneApplicationAttempt(
    ApplicationAttemptId applicationAttemptId,
    RMAppAttemptState rmAppAttemptFinalState, boolean keepContainers) {
    LOG.info(LOG_PREFIX + "Application Attempt " + applicationAttemptId + " is done." +
    ...
    if (application == null || attempt == null) {
      LOG.info(LOG_PREFIX + "Unknown application " + applicationAttemptId + " has completed!");
      return;
    }
    ...
    for (RMContainer rmContainer : attempt.getLiveContainers()) {
      if (keepContainers
        ...
        // restart.
        LOG.info(LOG_PREFIX + "Skip killing " + rmContainer.getContainerId());
        continue;
      }
      completedContainer(
        rmContainer,
  ...
  public Allocation allocate(ApplicationAttemptId applicationAttemptId,
    ...
    if (application == null) {
      LOG.info(LOG_PREFIX + "Calling allocate on removed " +
    ...
    synchronized (application) {
      ...
      if (application.isStopped()) {
        LOG.info(LOG_PREFIX + "Calling allocate on a stopped " +
      ...
      if (!ask.isEmpty()) {
        ...
        LOG.info(LOG_PREFIX + "allocate: pre-update" +
          " applicationAttemptId=" + applicationAttemptId +
          " application=" + application);
        application.showRequests();

        LOG.info(LOG_PREFIX + "Update application request: " + ask);
        // Update application requests
        application.updateResourceRequests(ask);

        LOG.info(LOG_PREFIX + "allocate: post-update");
        application.showRequests();
      }

      LOG.info(LOG_PREFIX + "allocate:" +
        " applicationAttemptId=" + applicationAttemptId +
        " #ask=" + ask.size());
  ...
  private synchronized void nodeUpdate(RMNode nm) {
    ...
    for (ContainerStatus completedContainer : completedContainers) {
      ...
      LOG.info(LOG_PREFIX + "Container FINISHED: " + containerId);
...
//      LOG.info(LOG_PREFIX + "Trying to fulfill reservation for application " +
...
//    if (node.getReservedContainer() == null) {
      if (Resources.greaterThanOrEqual(calculator, getClusterResources(),
        ...
        LOG.info(LOG_PREFIX + "Trying to schedule on node: " + node.getNodeName() +
          ", available: " + node.getAvailableResource());
        root.assignContainers(clusterResource, node);
      }
//    } else {
//      LOG.info(LOG_PREFIX + "Skipping scheduling since node " + node.getNodeID() +
...
//    }
  ...
  private void containerLaunchedOnNode(ContainerId containerId, FiCaSchedulerNode node) {
    ...
    if (application == null) {
      LOG.info(LOG_PREFIX + "Unknown application "
    ...
    LOG.info(LOG_PREFIX2 + "Launched container: " + containerId + " on node: " + node.getNodeName());
    if(!DummyHistory.finished) {
      DummyHistory.assignments.add(node.getNodeID());
    }
    ...
    switch (event.getType()) {
      ...
      case APP_ADDED: {
        ...
        if(applications.size() < 1) {
          ...
          LOG.info(LOG_PREFIX + "Rejecting application: " + appAddedEvent.getApplicationId());
          LOG.info(LOG_PREFIX + "Only 1 application is allowed: " + applications);
          this.rmContext.getDispatcher().getEventHandler().handle(new RMAppRejectedEvent(appAddedEvent.getApplicationId(), "Rejecting app"));
        }
      }
      break;
      case APP_REMOVED: {
      ...
      break;
      default:
        LOG.error(LOG_PREFIX + "Invalid eventtype " + event.getType() + ". Ignoring!");
    }
  }
  ...
  private synchronized void addNode(RMNode nodeManager) {
    ...
    ++numNodeManagers;
    LOG.info(LOG_PREFIX + "Added node " + nodeManager.getNodeAddress() +
  ...
  private synchronized void removeNode(RMNode nodeInfo) {
    ...
    LOG.info(LOG_PREFIX + "Removed node " + nodeInfo.getNodeAddress() +
      " clusterResource: " + clusterResource);
  }
  ...
  private synchronized void completedContainer(RMContainer rmContainer,
    ...
    if (rmContainer == null) {
      LOG.info(LOG_PREFIX + "Null container completed...");
      return;
    }
    ...
    if (application == null) {
      LOG.info(LOG_PREFIX + "Container " + container + " of" + " unknown application "
    ...
    LOG.info(LOG_PREFIX + "Application attempt " + application.getApplicationAttemptId()
  ...
  public FiCaSchedulerApp getCurrentAttemptForContainer(
    ...
    SchedulerApplication app =
      applications.get(containerId.getApplicationAttemptId().getApplicationId());
    if (app != null) {
      return (FiCaSchedulerApp) app.getCurrentAppAttempt();
    }
    return null;
  ...
  public void dropContainerReservation(RMContainer container) {
    LOG.info(LOG_PREFIX + "DROP_RESERVATION:" + container.toString());
    completedContainer(container,
      SchedulerUtils.createAbnormalContainerStatus(
        container.getContainerId(),
        SchedulerUtils.UNRESERVED_CONTAINER),
  ...
  public void preemptContainer(ApplicationAttemptId aid, RMContainer cont) {
    LOG.info(LOG_PREFIX + "PREEMPT_CONTAINER: application:" + aid.toString() +
  ...
  public void killContainer(RMContainer cont) {
    LOG.info(LOG_PREFIX + "KILL_CONTAINER: container" + cont.toString());
  ...
  public synchronized boolean checkAccess(UserGroupInformation callerUGI,
    ...
    CSQueue queue = getQueue(queueName);
    if (queue == null) {
      LOG.info(LOG_PREFIX + "ACL not found for queue access-type " + acl
        + " for queue " + queueName);
      return false;
    }
