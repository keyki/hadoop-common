...
public class ExtendedCapacityScheduler extends AbstractYarnScheduler
  implements PreemptableResourceScheduler, CapacitySchedulerContext,
  Configurable {

  private static final Log LOG = LogFactory.getLog(ExtendedCapacityScheduler.class);
  ...
  public ExtendedCapacityScheduler() {
  }
   ...
   * @param cs
   */
  static void schedule(ExtendedCapacityScheduler cs) {
  ...
  static class AsyncScheduleThread extends Thread {
    ...
    private final ExtendedCapacityScheduler cs;
    private AtomicBoolean runSchedules = new AtomicBoolean(false);

    public AsyncScheduleThread(ExtendedCapacityScheduler cs) {
      this.cs = cs;
      setDaemon(true);
    }
  ...
  @Lock(ExtendedCapacityScheduler.class)
  ...
  private synchronized void addApplicationAttempt(ApplicationAttemptId applicationAttemptId, boolean transferStateFromPreviousAttempt) {
    ...
    FiCaSchedulerApp attempt =
      new FiCaSchedulerApp(applicationAttemptId, application.getUser(), queue, queue.getActiveUsersManager(), rmContext);
  ...
  public synchronized String moveApplication(ApplicationId appId, String queueName) throws YarnException {
    ...
    FiCaSchedulerApp attempt = (FiCaSchedulerApp) app.getCurrentAppAttempt();
    // To serialize with #allocate, synchronize on app attempt
    synchronized (attempt) {
      LeafQueue oldQueue = (LeafQueue) app.getQueue();
      LeafQueue targetQueue = (LeafQueue) queues.get(queueName);
      if (targetQueue == null) {
  ...
  private void verifyMoveDoesNotViolateConstraints(FiCaSchedulerApp app, LeafQueue oldQueue, LeafQueue targetQueue) throws YarnException {
    ...
    CSQueue lowestCommonAncestor = findLowestCommonAncestorQueue(oldQueue, targetQueue);
  ...
  private CSQueue findLowestCommonAncestorQueue(CSQueue queue1, CSQueue queue2) {
    ...
    String name1 = queue1.getQueuePath();
    String name2 = queue2.getQueuePath();
    // We keep track of the last period we encounter to avoid returning root.apple
    // when the queues are
    // root.applepie and
    // root.appletart
    int lastPeriodIndex = -1;
    int beforeLastPeriodIndex = -1;
    for (int i = 0; i < Math.max(name1.length(), name2.length()); i++) {
      if (name1.length() <= i || name2.length() <= i || name1.charAt(i) != name2.charAt(i)) {
        if (beforeLastPeriodIndex == -1) {
          return queues.get(name1.substring(0, lastPeriodIndex));
        }
        return queues.get(name1.substring(beforeLastPeriodIndex + 1, lastPeriodIndex));
      } else if (name1.charAt(i) == '.') {
        beforeLastPeriodIndex = lastPeriodIndex;
        lastPeriodIndex = i;
      }
    }
    return queue1;
  ...
  public FiCaSchedulerApp getCurrentAttemptForContainer(ContainerId containerId) {
    SchedulerApplication app = applications.get(containerId.getApplicationAttemptId().getApplicationId());
    if (app != null) {
      return (FiCaSchedulerApp) app.getCurrentAppAttempt();
    }
    return null;
